        ...
        if ( (PoolReturned + PoolFree) == Pool->PoolSize )
        {
            if ( Pool->PoolFree.compare_exchange_strong(PoolFree, 0)  )
            {
                ...
                if ( Root->Alive ) {
                    ...
                    if ( numFreePools > (OCTANE_KEEP_POOLS+OCTANE_LAZY_GC) ) {
...
void operator delete( void *ptr ) {
    ThreadLocalAllocator::dealloc(ptr);
}
